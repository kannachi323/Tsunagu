...
var settings = []Setting{
	...
	{"downloads_dir", KindRuntime, TypeString, ScopeLive, editable, "Where downloaded chapters and episodes are stored. Empty = media_dir. Change via the app to migrate existing files.", strGet(func(c *Config) *string { return &c.DownloadsDir }), strSet(func(c *Config) *string { return &c.DownloadsDir })},
	{"local_source_dir", KindRuntime, TypeString, ScopeLive, editable, "Where hand-added local manga/anime/novels are read from. Empty = media_dir/local. Change via the app to migrate existing files.", strGet(func(c *Config) *string { return &c.LocalSourceDir }), strSet(func(c *Config) *string { return &c.LocalSourceDir })},
	...
	{"backup_interval_hours", KindRuntime, TypeInt, ScopeFullRestart, editable, "How often an automatic database backup is taken, in hours (0 = never).", intGet(func(c *Config) *int { return &c.BackupIntervalHours }), intSet(0, func(c *Config) *int { return &c.BackupIntervalHours })},
	{"backup_retention_count", KindRuntime, TypeInt, ScopeFullRestart, editable, "How many automatic backups to keep before pruning the oldest.", intGet(func(c *Config) *int { return &c.BackupRetentionCount }), intSet(1, func(c *Config) *int { return &c.BackupRetentionCount })},
	...
	{"manga_download_format", KindRuntime, TypeString, ScopeLive, editable, "How newly downloaded manga chapters are stored on disk: loose (individual image files) / cbz (one .cbz archive per chapter).", strGet(func(c *Config) *string { return &c.MangaDownloadFormat }), enumSet(func(c *Config) *string { return &c.MangaDownloadFormat }, "loose", "cbz")},
