...
func (r *mutationResolver) CreateTrackerStub(ctx context.Context, trackerKey string, remoteID string, contentType model.ContentType, title string, coverURL *string) (*model.Media, error) {
	...
	if key == "" || rid == "" {
		return nil, fmt.Errorf("trackerKey and remoteId are required")
	}
	if name == "" {
	...
	existing, err := r.Q.GetLocalMediaByExternalID(ctx, externalID)
	if err != nil && err != sql.ErrNoRows {
		return nil, err
	}

	var row sqlcgen.Medium
	if err == sql.ErrNoRows {
		row, err = r.Q.CreateSourcelessMedia(ctx, sqlcgen.CreateSourcelessMediaParams{
			ExtensionName: trackerStubLabel(key),
		...
		if existing.ExtensionName == "Local" {
		...
		if !row.AddedAt.Valid {
			row, err = r.Q.AddMediaToLibrary(ctx, row.ID)
			if err != nil {
				return nil, err
			}
		}
	...
	if _, err := r.Tk.Bind(ctx, key, row.ID, rid); err != nil {
		return nil, err
	}
	return toMedia(row, r.MediaDir), nil
}
