...
const createSourcelessMedia = `-- name: CreateSourcelessMedia :one
INSERT INTO media (
    extension_id, extension_name, external_id, content_type, title,
...
type CreateSourcelessMediaParams struct {
...
func (q *Queries) CreateSourcelessMedia(ctx context.Context, arg CreateSourcelessMediaParams) (Medium, error) {
	row := q.db.QueryRowContext(ctx, createSourcelessMedia,
		arg.ExtensionName,
		arg.ExternalID,
		arg.ContentType,
		arg.Title,
...
SELECT id, extension_id, extension_name, external_id, content_type, title, cover_path, cover_local_path, description, status, author, artist, extension_removed_at, added_at, last_viewed_at, details_fetched_at, updated_at, chapters_synced_at, cover_override, content_block_rank, metadata_search_failed_at FROM media WHERE extension_id IS NULL AND external_id = ?
`
...
const updateSourcelessMedia = `-- name: UpdateSourcelessMedia :one
UPDATE media SET
    title = ?,
...
type UpdateSourcelessMediaParams struct {
	Title     string         `json:"title"`
	CoverPath sql.NullString `json:"cover_path"`
	ID        int64          `json:"id"`
}

func (q *Queries) UpdateSourcelessMedia(ctx context.Context, arg UpdateSourcelessMediaParams) (Medium, error) {
	row := q.db.QueryRowContext(ctx, updateSourcelessMedia, arg.Title, arg.CoverPath, arg.ID)
	var i Medium
	err := row.Scan(
		&i.ID,
		&i.ExtensionID,
